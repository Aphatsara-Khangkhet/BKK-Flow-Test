...
**22 สไลด์ / 14 กราฟ**: สไลด์ 1–19 เป็นเรื่องหลัก และ 20–22 เป็นภาคผนวก

- [PDF พร้อมนำเสนอ](presentation/Traffic_ReView_Final_V5_Classroom.pdf)
- [PowerPoint แก้ไขกราฟและบทพูดได้](presentation/Traffic_ReView_Final_V5_Classroom.pptx)
- [บทพูดเรียงสไลด์](sample_report/Speaker_Notes.md)
- [Notebook สำหรับ Colab](Bangkok_Traffic_Final_V5_Colab.ipynb)
- [เปิดสไลด์ HTML](sample_report/Presentation.html)

เวอร์ชันนี้แยกจาก V4 และเวอร์ชันเดิม ใช้ไฟล์ `(5).xlsx` ชีต `traffic_data` ข้อมูลเป็นฉบับร่างที่ยังต้องทวนต้นทาง **Final หมายถึงชุดนำเสนอที่เรียงครบ ไม่ใช่การรับรองความถูกต้องของทุกค่าต้นทาง**

## ลำดับการเล่า

| สไลด์ | เนื้อหา |
| --- | --- |
| 1–3 | ที่มา คำถาม และหน่วยเปรียบเทียบ |
| 4–9 | Coverage ภาพรวมสามช่วง การกระจาย รูปแบบ และเวลาสำรวจ |
| 10–12 | ความต่างรายสถานที่ วันสำรวจจริง และประเภทรถของกรณีศึกษา |
| 13–14 | องค์ประกอบรถและการเปลี่ยนสัดส่วน |
| 15–16 | Lockdown และความไวต่อการจับคู่ |
| 17–19 | ข้อสรุป ข้อเสนอแนะ แหล่งข้อมูล และบทบาท AI |
| 20–22 | วันในสัปดาห์ นิยาม และคำถามที่คาดว่าจะได้รับ |

ใช้ทั้งชุดเมื่อต้องการนำเสนอฉบับเต็ม ถ้าเวลาสั้นมีลำดับย่อแนะนำใน Read.md เวลาจริงต้องยึดประกาศอาจารย์ ยังต้องเติมชื่อสมาชิกและการแบ่งหน้าที่ตามจริงก่อนส่งงานกลุ่ม

## เปลี่ยน Raw Data

บน Colab อัปโหลด notebook นี้ เลือกไฟล์ `.xlsx` ใหม่ ชีต `traffic_data` แล้วอ่าน audit ก่อนรันส่วนวิเคราะห์ โค้ดครบใน notebook ไม่ต้องหาโมดูล traffic_audit.py เพิ่ม มีฟอนต์ Noto Sans Thai และใบอนุญาต OFL รวมไว้แล้ว

บนเครื่อง:

```sh
python -m pip install -r requirements.txt
python src/final_project.py --input "path/to/new.xlsx"
```

แต่ละรอบได้โฟลเดอร์ใหม่: กราฟ PNG, ตาราง CSV, Read.md, บทพูด และ HTML สไลด์ ตัวเลขคำนวณใหม่จาก raw ที่เลือก ส่วน PPTX/PDF ใน presentation เป็น snapshot ของไฟล์ (5).xlsx ต้องอัปเดตเมื่อเปลี่ยน raw; ผล HTML ที่สร้างใหม่ใช้นำเสนอรอบใหม่ได้ทันที

ยังไม่รองรับการเปลี่ยน schema หรือกรณีที่จับคู่/กรณีศึกษาหรือ Lockdown ไม่ผ่านเกณฑ์โดยอัตโนมัติ โปรแกรมจะหยุดให้ทบทวน audit ไม่เอาผลจากไฟล์เก่ามาแทน

## สิ่งที่เพิ่มจาก V4

จาก 7 เป็น 14 กราฟ โดยเพิ่มการกระจายรายหน่วย รูปแบบการเปลี่ยนแปลง ช่วงเวลาสำรวจ วันสำรวจกรณีศึกษา การขยายความเปลี่ยนของสัดส่วน และวันในสัปดาห์ จัดเป็น 22 สไลด์พร้อมบทพูด ไม่ได้เพิ่มกราฟเพื่อซ้ำข้อสรุปเดิมอย่างเดียว

## ขอบเขตผล

แบ่งช่วงด้วย covid_period; Before 2017–2019, During 2020–2021, After 2022 ถึงวันล่าสุด 29 พ.ค. 2026 ไม่ใช้ปีล่าสุดเทียบยอดเต็มปี ใช้หน่วยทางแยก–ถนน–เวลาตรงกัน 721 หน่วยจาก 204 ทางแยก

ดัชนีหลัก Before=100, During=91.5, After=90.2 ไม่ใช่ยอดรถทั้งหมดในกรุงเทพฯ และไม่พิสูจน์เหตุและผลจากโควิด คง flag Lockdown ตามไฟล์ พร้อมเปิดเผย 168 แถวที่ต่างจากกรอบวันอ้างอิง
...
![ข้อมูลครอบคลุมวันใดบ้าง](sample_report/figures/01_coverage.png)
...
[ตารางคำนวณ](sample_report/tables/coverage.csv)

## กราฟ 02 — ปริมาณรถในหน่วยที่จับคู่ครบสามช่วง

![ปริมาณรถในหน่วยที่จับคู่ครบสามช่วง](sample_report/figures/02_overview.png)
...
[ตารางคำนวณ](sample_report/tables/overview.csv)

## กราฟ 03 — แต่ละหน่วยอยู่เหนือหรือต่ำกว่าฐาน

![แต่ละหน่วยอยู่เหนือหรือต่ำกว่าฐาน](sample_report/figures/03_paired.png)
...
[ตารางคำนวณ](sample_report/tables/matched_levels.csv)

## กราฟ 04 — ค่ากลางซ่อนการกระจายมากแค่ไหน

![ค่ากลางซ่อนการกระจายมากแค่ไหน](sample_report/figures/04_distribution.png)
...
[ตารางคำนวณ](sample_report/tables/index_distribution.csv)

## กราฟ 05 — การเปลี่ยนแปลงมีสี่รูปแบบ

![การเปลี่ยนแปลงมีสี่รูปแบบ](sample_report/figures/05_patterns.png)
...
[ตารางคำนวณ](sample_report/tables/period_patterns.csv)

## กราฟ 06 — ช่วงเวลาสำรวจให้ผลเหมือนกันหรือไม่

![ช่วงเวลาสำรวจให้ผลเหมือนกันหรือไม่](sample_report/figures/06_time_windows.png)
...
[ตารางคำนวณ](sample_report/tables/time_window_indices.csv)

## กราฟ 07 — สถานที่ที่มีความต่างเด่น

![สถานที่ที่มีความต่างเด่น](sample_report/figures/07_locations.png)
กราฟแสดง 12 คู่ทางแยก–ถนนในช่วงเช้าที่มีการเปลี่ยนแปลงเด่นเมื่อเทียบกับก่อนโควิด โดยกำหนดค่าก่อนโควิดเท่ากับ 100 สีฟ้าแสดงค่าต่ำกว่าฐาน ส่วนสีแดงแสดงค่าสูงกว่าฐาน พบว่าแต่ละพื้นที่มีรูปแบบการเปลี่ยนแปลงแตกต่างกัน บางพื้นที่เพิ่มขึ้น ขณะที่บางพื้นที่ลดลงอย่างชัดเจน กราฟนี้จึงใช้เพื่อระบุจุดที่น่าสนใจสำหรับนำไปตรวจสอบและวิเคราะห์รายละเอียดเพิ่มเติม ไม่ใช่การจัดอันดับความหนาแน่นหรือความติดขัดของการจราจร

## กราฟ 08 — วันที่สำรวจของกรณีศึกษา

![วันที่สำรวจของกรณีศึกษา](sample_report/figures/08_case_dates.png)
...
[ตารางคำนวณ](sample_report/tables/case_dates.csv)

## กราฟ 09 — ประเภทรถที่ประกอบเป็นความต่างของพื้นที่

![ประเภทรถที่ประกอบเป็นความต่างของพื้นที่](sample_report/figures/09_case_vehicles.png)
...
[ตารางคำนวณ](sample_report/tables/case_vehicle_three_periods.csv)

## กราฟ 10 — องค์ประกอบรถครบสามช่วง

![องค์ประกอบรถครบสามช่วง](sample_report/figures/10_mix.png)
...
[ตารางคำนวณ](sample_report/tables/mix_change.csv)

## กราฟ 11 — สัดส่วนหมวดเล็กเปลี่ยนอย่างไร

![สัดส่วนหมวดเล็กเปลี่ยนอย่างไร](sample_report/figures/11_mix_delta.png)
...
[ตารางคำนวณ](sample_report/tables/mix_delta.csv)

## กราฟ 12 — Lockdown ภายในช่วง During

![Lockdown ภายในช่วง During](sample_report/figures/12_lockdown.png)
...
[ตารางคำนวณ](sample_report/tables/lockdown_summary.csv)

## กราฟ 13 — ข้อสรุปเปลี่ยนเมื่อจับคู่ละเอียดขึ้นหรือไม่

![ข้อสรุปเปลี่ยนเมื่อจับคู่ละเอียดขึ้นหรือไม่](sample_report/figures/13_sensitivity.png)
...
[ตารางคำนวณ](sample_report/tables/sensitivity.csv)

## กราฟ 14 — วันสำรวจของกลุ่มจับคู่เหมือนกันหรือไม่

![วันสำรวจของกลุ่มจับคู่เหมือนกันหรือไม่](sample_report/figures/14_weekday.png)
...
[ตารางคำนวณ](sample_report/tables/weekday_counts.csv)
